...
const generateFeed = (render, type, posts, tags, categories, config, renderOptions, output, order_by, limit, helpers, log) => {
...
const baseGenerator = (type, hexo, generatorConfig, packageTemplateName, { posts, tags, categories }) => {
    ...
    return Promise.all([
        ...
        ...postsByTag.map(({ type, posts, output }) =>
            generateFeed(render, type, posts, tags, categories, config, renderOptions, output, order_by, itemCount, helpers, log)
        ),
        ...postsByCategory.map(({ type, posts, output }) =>
            generateFeed(render, type, posts, tags, categories, config, renderOptions, output, order_by, itemCount, helpers, log)
        )
